...
import 'package:flutter_sixvalley_ecommerce/features/prediction/prediction_hub_screen.dart';
import 'package:flutter_sixvalley_ecommerce/features/botcenter/screens/bot_chat_screen.dart';
import 'package:flutter_sixvalley_ecommerce/features/smartads/screens/smartads_notification_screen.dart';
...
class RouterHelper {
  ...
  static const String predictionHubScreen = '/prediction-hub';
  static const String botChatScreen = '/bot-chat';
  ...
  static String getBotChatRoute({RouteAction? action}) => _navigateRoute(botChatScreen, route: action);
  static String getSmartAdsNotificationRoute({RouteAction? action}) => _navigateRoute(smartAdsNotificationScreen, route: action);
  ...
  static String getPredictionHubRoute({RouteAction? action, int? matchId}) {
    final query = matchId != null ? '?match_id=$matchId' : '';
    return _navigateRoute('$predictionHubScreen$query', route: action);
  }
    ...
    routes: [
      ...
      GoRoute(path: dashboardScreen, builder: (context, state) {
        ...
        return DashBoardScreen(
          pageIndex: page == 'home' ? 0 : page == 'offers' ? 1 : page == 'cart' ? 2 : page == 'orders' ? 3 : page == 'more' ? 4 : 0,
        );
      }),
      ...
      GoRoute(
        path: otpLoginScreen,
        builder: (context, state) {
          final fromLogout = state.uri.queryParameters['formLogout'] == 'true';
      ...
      GoRoute(
        path: predictionHubScreen, 
        builder: (context, state) {
          final matchId = int.tryParse(state.uri.queryParameters['match_id'] ?? '');
          return PredictionHubScreen(initialMatchId: matchId);
        }
      ),
      GoRoute(
        path: botChatScreen,
      ...
      GoRoute(
        path: smartAdsNotificationScreen,
        builder: (context, state) => const SmartAdNotificationScreen(),
      ),
