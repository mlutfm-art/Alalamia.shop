...
class AddressInfoItem extends StatelessWidget {
  final String icon;
  final String title;
  const AddressInfoItem({super.key, required this.icon, required this.title});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(children: [
        SizedBox(width: 18, child: Image.asset(icon, color: Theme.of(context).primaryColor.withOpacity(.5))),
        const SizedBox(width: Dimensions.paddingSizeSmall),
        Expanded(child: Text(title, style: textRegular.copyWith(fontSize: Dimensions.fontSizeDefault), maxLines: 1, overflow: TextOverflow.ellipsis)),
      ]),
    );
  }
}
