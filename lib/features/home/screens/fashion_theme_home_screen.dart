...
class FashionThemeHomePage extends StatefulWidget {
  ...
  static Future<void> loadData(bool reload) async {
    ...
    if(Provider.of<AuthController>(Get.context!, listen: false).isLoggedIn()){
      if(profileController.userInfoModel == null) await profileController.getUserInfo(Get.context!);
...
class _FashionThemeHomePageState extends State<FashionThemeHomePage> {
  ...
  final List<ProductType> productTypeList = [
    ProductType.newArrival, ProductType.topProduct, ProductType.bestSelling, ProductType.discountedProduct,
  ];
          ...
          slivers: [
            ...
            SliverToBoxAdapter(
              child: Column(children: [
                ...
                // Prediction Banner removed to hide it from home screen

                Consumer<FlashDealController>(
                    builder: (context, megaDeal, child) {
                      return (megaDeal.flashDeal != null && megaDeal.flashDeal!.id != -1) ? megaDeal.flashDealList.isNotEmpty ?
                      Column(children: [
                        Padding(
                      ...
                      ]) : const SizedBox.shrink() : (megaDeal.flashDeal == null) ? const FlashDealShimmer() : const SizedBox.shrink();
                    }),
                ...
                Consumer<BannerController>(
                  builder: (context, bannerProvider, child) {
                    return (bannerProvider.promoBannerMiddleTop != null && bannerProvider.promoBannerMiddleTop!.id != -1) ? Padding(
                ...
                Consumer<BannerController>(builder: (context, bannerProvider, child){
                  return (bannerProvider.promoBannerLeft != null && bannerProvider.promoBannerLeft!.id != -1) ? Padding(
                ...
                Consumer<BannerController>(builder: (context, bannerProvider, child){
                  return (bannerProvider.promoBannerMiddleBottom != null && bannerProvider.promoBannerMiddleBottom!.id != -1) ?
                ...
                Consumer<ProductController>(
                    builder: (context, demandProvider, _) {
                      return (demandProvider.mostDemandedProductModel != null && demandProvider.mostDemandedProductModel!.id != -1)?
                ...
                Consumer<BannerController>(builder: (context, bannerProvider, child){
                  return (bannerProvider.promoBannerRight != null && bannerProvider.promoBannerRight!.id != -1) ?
                ...
                Consumer<BannerController>(builder: (context, bannerProvider, child){
                  return (bannerProvider.promoBannerBottom != null && bannerProvider.promoBannerBottom!.id != -1)?
                  Padding(padding: const EdgeInsets.only(bottom: Dimensions.homePagePadding ),
...
class _AllProductTypeTabWidget extends StatelessWidget {
  const _AllProductTypeTabWidget({required this.productTypeList});
  final List<ProductType> productTypeList;
                    ...
                    return InkWell(
                        ...
                        child: Container(
                            ...
                            child: Center(child: Text(_getFilterTypeTitle(productTypeList[index], context), style: textMedium.copyWith(
                              color: productTypeList[index] == productType ? Theme.of(context).primaryColor : Theme.of(context).hintColor,
                            ))))
                    );
                  }),
              ),
  ...
  String _getFilterTypeTitle(ProductType type, BuildContext context) {
    switch (type) {
      case ProductType.newArrival: return getTranslated('new_arrival',context)!;
      case ProductType.topProduct: return getTranslated('top_product',context)!;
      case ProductType.bestSelling: return getTranslated('best_selling',context)!;
      case ProductType.discountedProduct: return getTranslated('discounted_product',context)!;
...
class SliverDelegate extends SliverPersistentHeaderDelegate {
  ...
  @override
  bool shouldRebuild(SliverDelegate oldDelegate) => oldDelegate.maxExtent != height || oldDelegate.minExtent != height || child != oldDelegate.child;
}
