...
class AddressRepository implements AddressRepoInterface<ApiResponseModel>{
  ...
  Future<ApiResponseModel> add(AddressModel addressModel) async {
    try {
      String guestId = Provider.of<AuthController>(Get.context!, listen: false).getGuestToken() ?? '';
      Response response = await dioClient!.post(
        '${AppConstants.addAddressUri}${guestId.isNotEmpty ? '?guest_id=$guestId' : ''}', 
  ...
  Future<ApiResponseModel> update(Map<String, dynamic> addressModel, int addressId) async {
    try {
      String guestId = Provider.of<AuthController>(Get.context!, listen: false).getGuestToken() ?? '';
      Response response = await dioClient!.post(
        '${AppConstants.updateAddressUri}${guestId.isNotEmpty ? '?guest_id=$guestId' : ''}', 
        data: addressModel
      );
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
