...
class DashBoardScreenState extends State<DashBoardScreen> {
  ...
  List<NavigationModel> _getScreens(SplashController splashController) {
    return [
      NavigationModel(
        name: 'home',
      ...
      // Offers with the Medal Icon explicitly set here
      NavigationModel(
          name: 'offers', 
          icon: Images.offers, 
          screen: const OfferProductListScreen()
      ),
  ...
  Widget build(BuildContext context) {
    ...
    final List<NavigationModel> screens = _getScreens(splashController);
    ...
    return PopScope(canPop: false,
      ...
      child: Scaffold(
        key: _scaffoldKey,
        body: PageStorage(bucket: bucket, child: screens[_pageIndex].screen),
        bottomNavigationBar: Container(height: 68,
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: _getBottomWidget(screens)))));
  }
  ...
  List<Widget> _getBottomWidget(List<NavigationModel> screens) {
    List<Widget> list = [];
    for(int index = 0; index < screens.length; index++) {
      list.add(Expanded(child: CustomMenuWidget(
        key: ValueKey('footer_item_${screens[index].name}_$index'),
