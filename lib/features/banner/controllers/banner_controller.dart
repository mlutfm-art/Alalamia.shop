...
class BannerController extends ChangeNotifier {
  ...
  Future<void> getBannerList() async {
    // نضع قوائم فارغة مبدئياً لتجنب الشيمر اللانهائي في حال الفشل
    _mainBannerList ??= [];
    _footerBannerList ??= [];
  ...
  void clickBannerRedirect(BuildContext context, int? id, Product? product,  String? type, {String? url}) {
    ...
    if(type == 'custom' && url != null && url.isNotEmpty) {
      ...
      if(cIndex != -1 && Provider.of<CategoryController>(context, listen: false).categoryList[cIndex].name != null){
      ...
      if(bIndex != -1 && Provider.of<BrandController>(context, listen: false).brandList[bIndex].name != null){
      ...
      if(tIndex != -1 && Provider.of<ShopController>(context, listen: false).allSellerModel?.sellers?[tIndex].shop?.name != null){
