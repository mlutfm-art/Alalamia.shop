...
class ProductController extends ChangeNotifier {
    ...
    if(offset == 1) {
      ...
      Future.delayed(const Duration(seconds: 5), () {
        if(_selectedProductModel == null) {
          _selectedProductModel = ProductModel(products: [], offset: 1);
          notifyListeners();
        }
      });
    }else {
  ...
  Future<void> getLatestProductList(int offset, {bool isUpdate = false}) async {
    ...
    if(offset == 1) {
      ...
      Future.delayed(const Duration(seconds: 5), () {
        if(_latestProductModel == null) {
          _latestProductModel = ProductModel(products: [], offset: offset);
          notifyListeners();
        }
      });
    }else {
  ...
  Future<void> getClearanceAllProductList(int offset, {bool isUpdate = true}) async {
    ...
    if(offset == 1) {
      ...
      Future.delayed(const Duration(seconds: 5), () {
        if(_clearanceProductModel == null) {
          _clearanceProductModel = ProductModel(products: [], offset: offset);
          notifyListeners();
        }
      });
  ...
  Future <ApiResponseModel> getClearanceSearchProduct({required String query, String? categoryIds, String? brandIds,  String? authorIds, String? publishingIds, String? sort, String? priceMin, String? priceMax, required int offset, String? productType, String offerType = 'clearance_sale', bool fromPaginantion = false, isNotify = true, dynamic l}) async {
