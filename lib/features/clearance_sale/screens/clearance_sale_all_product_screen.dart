...
class _ClearanceSaleAllProductScreenState extends State<ClearanceSaleAllProductScreen> {
  ...
  void initState() {
    ...
    Provider.of<ProductController>(Get.context!, listen: false).getClearanceSearchProduct(offset: 1, query: '', isNotify: false, l: null);
  }
...
void applyFilter({required BuildContext context, bool fromPagination = false, int? offset}) {
  ...
  if(fromPagination) {
    productController.getClearanceSearchProduct(query: productController.isSearchActive ? productController.searchText ?? '' : '', categoryIds: selectedCategoryId,
        ...
        productType:  searchProvider.productTypeIndex == 0 ? 'all' : searchProvider.productTypeIndex == 1 ? 'physical' : 'digital', offerType: 'clearance_sale', fromPaginantion: fromPagination, l: null
    );
  } else {
    ...
    productController.getClearanceSearchProduct(query: productController.isSearchActive ? productController.searchText ?? '' : '',categoryIds: selectedCategoryId,
        ...
        productType:  searchProvider.productTypeIndex == 0 ? 'all' : searchProvider.productTypeIndex == 1 ? 'physical' : 'digital', offerType: 'clearance_sale', l: null
    ).then((value) {
...
void onClick( BuildContext context, ProductController productController, TextEditingController searchController){
  if(searchController.text.isEmpty && !productController.isSearchActive) {
    ...
    productController.getClearanceSearchProduct(offset: 1, query: productController.searchText!, l: null);
    ...
    productController.toggleSearchActive();
    productController.getClearanceSearchProduct(offset: 1, query: '', l: null);
  }
}
